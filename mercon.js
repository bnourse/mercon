...
async function main() {
  ...
  try {
    ...
    if (!clipboardContent.trim()) {
      console.error("Clipboard is empty. Please copy a Mermaid diagram first.");
      process.exit(1);
    }
    
    // Create temporary Mermaid file
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'mermaid-converter-'));
    tempMermaidPath = path.join(tempDir, 'input.mmd');
    ...
    // Cleanup temp files
    if (tempMermaidPath && fs.existsSync(tempMermaidPath)) {
